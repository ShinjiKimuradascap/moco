...
# グローバルなアーティファクトストレージ（ロック付き）
# スレッド間で共有されるため、threading.local()ではなくグローバル変数を使用
_artifacts_lock = threading.Lock()
_pending_artifacts: List[Dict[str, Any]] = []


def _get_artifacts() -> List[Dict[str, Any]]:
    """アーティファクトリストを取得"""
    return _pending_artifacts
...
def _get_pending_artifacts() -> List[Dict[str, Any]]:
...
def _add_artifact(artifact: Dict[str, Any]) -> None:
    """アーティファクトを追加"""
    global _pending_artifacts
    with _artifacts_lock:
