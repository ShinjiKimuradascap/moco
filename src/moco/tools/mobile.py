...
def send_image_to_whatsapp(path: str, caption: str = "") -> str:
    """
    Send an image to the connected WhatsApp client.
    This tool sends generated charts, diagrams, or analysis results to the user's mobile device via WhatsApp.
    
    Args:
        path (str): Path to the image file (relative to workspace or absolute).
        caption (str, optional): Caption for the image.
    
    Returns:
        str: Status message with a magic tag that triggers the actual sending on the client side.
    """
    import os
    try:
        # Resolve path safely
        ...
        # Return magic tag
        # The client monitors for [IMAGE: path] pattern in the final response.
        return f"Image staged for delivery.\n[IMAGE: {abs_path}]"
